...
class Companion:
    ...
    def __init_wpa_supplicant(self):
        print('[*] Tool Running💃Working Only WPS Enable Router')
        print('[*] This Tool Create🧘Mohammad Abdullah-😎')
        print('[*] No Misuse Only For Educational Purpose')
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        ...
        print(f"[+] সোনার হরিণ 👉: '{wpa_psk}'")
        
        print(f"[+] 🍀🌱🌴🪴🌱🌼⛄🌻🌱☘️")
        
        print(f"[+] Wifi Name🪺: '{essid}'")
        
        print(f"[+] Joy Bangla Joy Bongobondu")
    ...
    def single_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, showpixiecmd=False,
        ...
        if self.connection_status.status == 'GOT_PSK':
            ...
            if self.pixie_creds.got_all():
                ...
                print('[!] Not enough data to run Pixie Dust attack router one of Holei Kaj Korbe')
                return False
        else:
...
class WiFiScanner:
    ...
    def prompt_network(self) -> str:
        ...
        while 1:
            try:
                networkNo = input('   👆kontar Password Lagbo Go  (press Enter to refresh): ')
